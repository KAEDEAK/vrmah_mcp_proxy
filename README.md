...
主に Claude Code CLI / VSCode 拡張 / Codex CLI のいずれの環境でも同一の `mcp_server.py` で動作します。
...
```
{
  "mcpServers": {
    "vrm_proxy_vscode": {
      "command": "python",
      "args": [
        "C:/path/to/VRMAgentHost/mcp_proxy/mcp_server.py"
      ]
    }
  }
}
...
###  接続イメージ
```
VSCode, LLM CLI などの環境 <--> mcp_proxy <--> 各サーバー(vrmah,voicevox)
```
...
mcp_proxy\spec\mcp_setup_*.txt を参考に VRM Agent Host の MCPの設定をして。
```
```
...
- ドキュメント中、以下の表現は、IPアドレスが入ります。例：192.168.1.2 
<ip-address-vrmah>
<ip-address-voicevox>
...
`mcp_proxy/config.json` を環境に合わせて編集:
...
4. **mcp_server.py** が、うまく呼び出せない場合は、フルパスで指定してください
...
└── spec/
    ...
    └── mcp_setup_for_vscode.txt  # VSCode 拡張用セットアップ
```
